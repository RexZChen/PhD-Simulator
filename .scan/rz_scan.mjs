...
import { activeContacts } from '/Users/rexzchen/Codes/PhDSim/src/engine/network.js';
...
let __k = 0;
const resolveAll = s => { let n = 0; while (s.event && n++ < 40) { const e = templateById[s.event]; globalThis.__SEEN.add(e.id); const pool = e.choices.filter(c => !c.ending && !c.minigame && !(c.requiresCoursework && s.coursework < c.requiresCoursework)); const ok = (pool.length ? pool[(__k++) % pool.length] : e.choices[0]); globalThis.__CH.add(e.id + '::' + ok.id); s = dispatch(s, { type: 'CHOICE', id: ok.id }); for (const a of (s.achievements||[])) globalThis.__AW.add(a); if (s.stage === 'minigame') s = dispatch(s, { type: 'LECTURE', worked: 9, attention: 5, caught: 1 }); } return s; };
