...
import { sceneDialog, milestoneDialog, pushbackDialog, lectureDialog, benchDialog, crisisDialog, vivaDialog, clusterDialog, patentDialog, summonsDialog, photoDialog, boardDialog, decisionDialog, offerDialog } from './scenes.js';
...
export function shell(run, ui, meta, saved, notices = []) {
  ...
  const menuRight = playing ? `${esc(stamp(s))}${s.tempo === 'week' ? ` · ${t('wk {n}', { n: Math.min(4, s.week + 1) })}` : ''}` : ui.screen === 'home' ? t('Please read the license. It is short.') : s ? phaseLabel[s.phase] || '' : '';
  ...
  <nav class="desktop-icons" aria-label="${esc(t('Desktop'))}">${applying ? `<button class="desk-icon ${!ui.app || !PRE_ENROL.includes(ui.app) ? 'active' : ''}" data-action="open" data-app="gradapply" title="GradApply 98">${icon('doc', 32)}<span>GradApply</span></button>` : ''}${appDefs.map(([id, ic2, label]) => `<button class="desk-icon ${openable(id) && ui.app === id ? 'active' : ''}" data-action="open" data-app="${id}" ${openable(id) ? '' : 'disabled'} title="${esc(lockedWhy(id) || t(label))}">${icon(ic2, 32)}<span>${t(label)}</span>${id === 'mail' && unreadMail ? `<b class="badge">${unreadMail}</b>` : ''}${id === 'chat' && unreadChat ? `<b class="badge" title="${esc(t('{n} unread message(s)', { n: unreadChat }))}">${unreadChat}</b>` : ''}${id === 'chat' && !unreadChat && openReq ? `<b class="badge req" title="${esc(t('{n} open request(s) from your advisor', { n: openReq }))}">!</b>` : ''}</button>`).join('')}<button class="desk-icon" data-action="collection" title="${esc(t('Achievements & discoveries'))}">${icon('star', 32)}<span>${t('Trophy Case')}</span></button></nav>
