import { activityPauseDialog } from './activity.js';
import { esc, btn, titlebar, menubar, statusbar, tag, money, fillBand, stamp } from './helpers.js';
import { accessibilityDialog } from './accessibility.js';
...
import { advisorPresence } from '../engine/conversation.js';
...
import { setupWizard, endingApp, collectionApp, aboutDialog, tipsDialog, confirmDialog, shutdownDialog, savesDialog } from './screens.js';
import { gradApply, threadDialog } from './apps/gradapply.js';
import { sceneDialog, scenePauseDialog, milestoneDialog, pushbackDialog, lectureDialog, benchDialog, crisisDialog, vivaDialog, clusterDialog, patentDialog, summonsDialog, photoDialog, boardDialog, decisionDialog, offerDialog } from './scenes.js';
...
function windowMenus(s, ui, meta, playing) {
  ...
  return [
    { id: 'file', label: t('File'), items: [
      ...
      { label: t('Saved runs…'), action: 'saves', hint: t('Load, overwrite, delete') },
      { sep: true },
...
function windowTitle(s, ui) {
  ...
  if (s?.stage === 'commencement') return [s.thesis?.deferred ? t('Degree conferred') : t('Commencement'), 'star'];
...
function content(s, ui, meta, saved, notices) {
  ...
  if (s.stage === 'trip' && s.trip) return tripScreen(s, { ...ui, selfPaced: !!meta.settings.selfPaced });
...
export function shell(run, ui, meta, saved, notices = []) {
  ...
  const currentApp = ui.screen === 'game' ? (applying && (!ui.app || !PRE_ENROL.includes(ui.app)) ? 'gradapply' : ui.app || 'dashboard') : null;
  const navApp = id => {
    const [, appIcon, label] = appDefs.find(app => app[0] === id);
    const active = openable(id) && currentApp === id;
    return `<button class="desk-icon ${active ? 'active' : ''}" data-action="open" data-app="${id}" ${active ? 'aria-current="page"' : ''} ${openable(id) ? '' : 'disabled'} title="${esc(lockedWhy(id) || t(label))}">${icon(appIcon, 24)}<span>${esc(t(label))}</span>${id === 'mail' && unreadMail ? `<b class="badge">${unreadMail}</b>` : ''}${id === 'chat' && unreadChat ? `<b class="badge" title="${esc(t('{n} unread message(s)', { n: unreadChat }))}">${unreadChat}</b>` : ''}${id === 'chat' && !unreadChat && openReq ? `<b class="badge req" title="${esc(t('{n} open request(s) from your advisor', { n: openReq }))}">!</b>` : ''}</button>`;
  };
  const navGroup = (label, apps) => `<div class="workspace-nav-group"><div class="workspace-nav-label">${esc(label)}</div>${apps.map(navApp).join('')}</div>`;
  const workspaceNav = `<div class="workspace-brand">${icon('computer', 24)}<div><b>ACADEMIC OS</b><small>${t('Your workspace')}</small></div></div>
    ${applying ? `<div class="workspace-nav-group"><button class="desk-icon ${currentApp === 'gradapply' ? 'active' : ''}" ${currentApp === 'gradapply' ? 'aria-current="page"' : ''} data-action="open" data-app="gradapply" title="GradApply 98">${icon('doc', 24)}<span>GradApply</span></button></div>` : ''}
    ${navGroup(t('Your work'), ['dashboard', 'browser', 'scholar'])}
    ${navGroup(t('People'), ['mail', 'chat'])}
    ${navGroup(t('Campus'), ['portal', 'calendar'])}
    <div class="workspace-nav-group"><div class="workspace-nav-label">${t('Personal')}</div>${['life', 'status'].map(navApp).join('')}<button class="desk-icon ${ui.screen === 'collection' ? 'active' : ''}" ${ui.screen === 'collection' ? 'aria-current="page"' : ''} data-action="collection" title="${esc(t('Achievements & discoveries'))}">${icon('star', 24)}<span>${t('Trophy Case')}</span></button></div>`;
  const phaseLabel = { prep: t('Fall 2027'), application: t('December 2027'), interviews: t('January–March 2028'), admissions: t('April 2028') };
  const menuRight = playing ? `${esc(stamp(s))}${s.tempo === 'week' ? ` · ${t('wk {n}', { n: Math.min(4, s.week + 1) })}` : ''}` : ui.screen === 'home' ? t('Your story starts here.') : s ? phaseLabel[s.phase] || '' : '';
  const isWizard = ui.screen === 'home' || (!s && ui.screen !== 'collection');
  const mode = playing ? MODES[s.advisorMode?.id || 'normal'] : null;
  const trayDate = playing ? esc(stamp(s)) : ui.screen === 'home' ? t('September 2027') : s ? esc(stamp(s)) : '09:41';
  ...
  <nav class="desktop-icons workspace-nav" aria-label="${esc(t('Desktop'))}">${workspaceNav}</nav>
  <main class="workspace ${playing ? '' : 'no-side'}">
    ...
    <div class="mini"><div class="titlebar"><span class="tb-title">${icon('user', 14)}<span>${t('Prof. {name}', { name: lastName(s.advisor.name) })}</span></span></div><div class="body"><div class="presence ${['checkedOut', 'traveling'].includes(s.advisorMode?.id) ? 'off' : s.advisorMode?.id === 'grant' ? 'away' : s.advisorMode?.id === 'pressed' ? 'typing' : ''}"><i></i>${esc(advisorPresence(s))}</div><div class="small muted">${esc(t(mode.label))} · ${t('1:1s {cadence}', { cadence: t(s.cadence.oneOnOne) })}</div>${openReq ? `<div class="small" style="margin-top:4px">${tag(t('{n} open request(s)', { n: openReq }), 'warn')}</div>` : ''}<div class="row" style="margin-top:6px">${btn(t('Message'), 'open', { app: 'chat', cls: 'small' })}${btn(t('Requests'), 'open', { app: 'dashboard', cls: 'small link' })}</div></div></div>
  ...
  ${ui.screen === 'game' && s?.event && !s?.pushback ? sceneDialog(s) : ''}${ui.screen === 'game' && s?.pushback ? pushbackDialog(s) : ''}${ui.screen === 'game' && s?.stage === 'minigame' && s?.minigame === 'lecture' ? lectureDialog(s, meta.settings) : ''}${ui.screen === 'game' && s?.stage === 'minigame' && s?.minigame === 'bench' ? benchDialog(s, meta.settings) : ''}${ui.screen === 'game' && s?.stage === 'minigame' && s?.minigame === 'viva' ? vivaDialog(s) : ''}${ui.screen === 'game' && s?.stage === 'minigame' && s?.minigame === 'cluster' ? clusterDialog(meta.settings) : ''}${ui.screen === 'game' && s?.stage === 'crisis' ? crisisDialog(s) : ''}${ui.screen === 'game' && s?.stage === 'summons' ? summonsDialog(s) : ''}${ui.screen === 'game' && s?.photo ? photoDialog(s) : ''}${ui.screen === 'game' && ui.board && s ? boardDialog(s) : ''}${ui.screen === 'game' && ui.decision && s ? decisionDialog(s, ui) : ''}${ui.screen === 'game' && ui.offer && s ? offerDialog(s, ui) : ''}${(ui.dialog === 'pause' || ui.comfortReturn === 'pause') && s ? scenePauseDialog(s) : ''}${(ui.dialog === 'activity-pause' || ui.comfortReturn === 'activity-pause') ? activityPauseDialog() : ''}${ui.saves ? savesDialog(ui.saves.slots, s, ui) : ''}${playing && !s.event && s.stage === 'plan' && s.patent && ['meetings', 'action'].includes(s.patent.stage) ? patentDialog(s) : ''}${ui.screen === 'game' && s && !s.event && ui.thread ? threadDialog(s, ui) : ''}${playing && s.stage === 'report' && !s.event ? reportDialog(s) : ''}${playing && s.stage === 'milestone' && !s.event ? milestoneDialog(s) : ''}
  ${ui.confirm ? confirmDialog(ui) : ''}${ui.dialog === 'accessibility' ? accessibilityDialog(meta.settings) : ''}${ui.dialog === 'about' ? aboutDialog() : ''}${ui.dialog === 'tips' ? tipsDialog(s) : ''}${ui.dialog === 'shutdown' ? shutdownDialog() : ''}
  </div>`;
}
