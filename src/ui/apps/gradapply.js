...
function header(s, ui, title, sub, action = '') {
  ...
  <div class="tabs ga-tabs" style="margin-bottom:8px">${tabDefs().map(([id, label, when]) => btn(`${label} <span class="tiny muted">${when}</span>`, 'ga-tab', { id, disabled: id === 'admissions' && unopened(s).length > 0, title: id === 'admissions' && unopened(s).length > 0 ? t('Read your updates first.') : '', cls: (unopened(s).length ? 'interviews' : (ui.gaTab || s.phase)) === id ? 'active' : '', disabled: phases.indexOf(id) > phases.indexOf(s.phase) })).join('')}</div>
  ${guideStrip(s, ui)}`;
}
...
export function status(s, ui) {
  ...
  ${unopened(s).length ? '' : note(pending.length ? t('Interview invitations arrived. Each is a short video call with your professor of interest: four questions, then a decision you will not hear for weeks.') : t('Nothing to do but wait. Decisions arrive in March; waitlists move in April.'))}
