...
const currentTab = (s, ui) => unopened(s).length ? 'interviews'
  : phases.includes(ui.gaTab) && phases.indexOf(ui.gaTab) <= phases.indexOf(s.phase) ? ui.gaTab : s.phase;
...
function guideStrip(s, ui) {
  ...
  const g = tab !== s.phase && !unopened(s).length
    ? { title: t('You are reviewing an earlier step'), detail: t('Your next task is in {tab}. Return there to continue.', { tab: phaseLabel }), action: 'ga-tab', id: s.phase, label: t('Return to {tab}', { tab: phaseLabel }) }
    : nextStep(s);
  if (!g) return '';
  const cta = g.action === 'prep' && g.id
    ...
    : g.action && g.action !== 'prep'
      ? btn(g.label, g.action, { id: g.id, cls: 'primary small', attrs: 'data-guide="1"' })
...
const researched = (s, sc) => {
  ...
  return `${facts}<details class="application-details" data-detail="application-campus-${esc(sc.id)}"><summary>${t('Campus & student notes')}</summary>${vibe}${notes.length ? `<div class="insider"><b class="small">${t('What people who are there say')}</b><ul>${notes.map(n => `<li>“${esc(t(n.line))}” <span class="src">— ${esc(t(n.from))}</span></li>`).join('')}</ul></div>` : ''}</details>`;
};
...
export function status(s, ui) {
  ...
  return `${header(s, ui, t('GradApply — Status'), t('January to March 2028. Interviews, waiting, refreshing. The portal has one button and it is “Refresh.”'), s.phase !== 'interviews' ? '' : pending.length ? `<span class="small">${t('{n} interview(s) to do', { n: pending.length })}</span>` : btn(t('Refresh the portal (March decisions) →'), 'decisions', { cls: 'primary continue' }))}
  ...
  ${unopened(s).length ? `<div class="updates" data-application-section="updates" tabindex="-1"><b>${t('{n} update(s) waiting', { n: unopened(s).length })}</b><p class="tiny muted">${esc(t(portalNote))}</p><div class="up-row">${unopened(s).map(a => { const sc = schools.find(x => x.id === a.schoolId); return `<button class="btn update" data-action="decision-open" data-id="${a.schoolId}">${crest(sc, 18)}<span><b>${esc(sc.name)}</b><small>${t('View update')}</small></span></button>`; }).join('')}</div></div>` : ''}
  <div class="listview apply-table" data-application-section="interviews" tabindex="-1"><div class="lv-head"><span>${t('Program')}</span><span>${t('Professor of interest')}</span><span>${t('Submitted')}</span><span>${t('Letters')}</span><span>${t('Status')}</span><span></span></div>${s.applications.map(app => { const sc = schools.find(x => x.id === app.schoolId); const poi = s.advisors.find(a => a.id === app.poiId); const late = s.prep.letters.filter(l => l.asked && l.status === 'late').length; return `<div class="lv-row"><span class="school-cell">${crest(sc, 30)}<b>${esc(sc.name)}</b></span><span class="small">${avatar(poi.id, 18)} ${esc(poi.name)}</span><span class="small">Dec 15 · ${t(app.effort)}</span><span class="small">${late ? t('{n} late', { n: late }) : t('on time')}</span><span>${tag(sealed(app) ? t('update waiting') : t(app.status), sealed(app) ? 'warn' : app.status === 'admitted' ? 'ok' : app.status === 'rejected' ? 'bad' : app.status === 'interview' ? 'warn' : 'info')}</span><span>${app.interview && !app.interview.done ? btn(t('Join the call'), 'ga-thread', { id: `${app.schoolId}:interview`, cls: 'small primary' }) : app.interview ? btn(t('Transcript'), 'ga-thread', { id: `${app.schoolId}:interview`, cls: 'small link' }) : ''}</span></div>`; }).join('')}</div>`;
}
