...
export function prepare(s, ui) {
  ...
  ${group(t('Fees'), p.waiverRolled ? `<p class="small">${p.waivers ? t('Waivers approved: applications are free.') : t('Waiver denied. $75 each, payable to a portal.')}</p>` : `<p class="small muted">${t('$75 per application unless waived. Waivers go to people who fill out the right form on the right site.')}</p>${btn(t('Request fee waivers (−3 Energy)'), 'prep', { id: 'waiver', cls: 'small' })}`)}</div>
  <div>${(() => {
...
export function programs(s, ui) {
  ...
  ${group(t('Settings for the next application'), `<div class="row"><label class="field"><span>${t('Effort')}</span><select id="effort-select">${[['generic', t('Generic application · 3 Energy')], ['tailored', t('Tailored statement · 7 Energy · better odds')]].map(([v, l]) => `<option value="${v}" ${effort === v ? 'selected' : ''}>${l}</option>`).join('')}</select></label><label class="check" style="margin-top:14px"><input id="contact-faculty" type="checkbox" ${contact ? 'checked' : ''}> ${t('Mention the email you sent (+3 Energy)')}</label><span class="small muted" style="margin-left:auto">${t('Next')}: ${cost.money ? money(cost.money) : t('fee waived')} · ${t('{n} Energy', { n: cost.energy })}</span></div><div class="row" style="margin-top:6px"><span class="small muted">${t('Show')}:</span>${[['all', t('All 22')], ['mine', t('My area')], ['applied', t('Applied')]].map(([id, l]) => btn(l, 'ga-filter', { id, cls: `small ${filter === id ? 'primary' : ''}` })).join('')}</div>`)}
...
export function status(s, ui) {
  ...
  <div class="listview apply-table"><div class="lv-head"><span>${t('Program')}</span><span>${t('Professor of interest')}</span><span>${t('Submitted')}</span><span>${t('Letters')}</span><span>${t('Status')}</span><span></span></div>${s.applications.map(app => { const sc = schools.find(x => x.id === app.schoolId); const poi = s.advisors.find(a => a.id === app.poiId); const late = s.prep.letters.filter(l => l.asked && l.status === 'late').length; return `<div class="lv-row"><span class="school-cell">${crest(sc, 30)}<b>${esc(sc.name)}</b></span><span class="small">${avatar(poi.id, 18)} ${esc(poi.name)}</span><span class="small">Dec 15 · ${t(app.effort)}</span><span class="small">${late ? t('{n} late', { n: late }) : t('on time')}</span><span>${tag(t(app.status), app.status === 'admitted' ? 'ok' : app.status === 'rejected' ? 'bad' : app.status === 'interview' ? 'warn' : 'info')}</span><span>${app.interview && !app.interview.done ? btn(t('Join the call'), 'ga-thread', { id: `${app.schoolId}:interview`, cls: 'small primary' }) : app.interview ? btn(t('Transcript'), 'ga-thread', { id: `${app.schoolId}:interview`, cls: 'small link' }) : ''}</span></div>`; }).join('')}</div>`;
}
...
export function offers(s, ui) {
  ...
  <div class="row" style="margin-bottom:8px">${s.applications.map(a => { const sc = schools.find(x => x.id === a.schoolId); return tag(`${sc.name}: ${t(a.status)}`, a.status === 'admitted' ? 'ok' : a.status === 'waitlisted' ? 'warn' : 'bad'); }).join('')}</div>
