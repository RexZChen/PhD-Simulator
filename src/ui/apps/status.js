...
import { personality, lastName, activeLabmates, peerAffiliation } from '../../engine/state.js';
...
export function statusApp(s, ui) {
  ...
  ${group(t('The lab'), `<div class="listview people-list"><div class="lv-head"><span>${t('Name')}</span><span>${t('Role')}</span><span>${t('Trait')}</span><span>${t('Bond')}</span></div>${activeLabmates(s).map(l => `<div class="lv-row"><span>${esc(l.name)}</span><span class="small">${esc(roleOf(l.role))}</span><span class="small">${esc(traitOf(l.trait))}</span><span class="small">${bondWord(l.bond)}</span></div>`).join('')}</div>`)}
  ${group(t('Cohort (other labs)'), `<div class="listview people-list"><div class="lv-head"><span>${t('Name')}</span><span>${t('Lab')}</span><span>${t('Status')}</span><span>${t('Bond')}</span></div>${s.peers.map(p => `<div class="lv-row"><span>${esc(p.name)}</span><span class="small">${t('Prof. {name}', { name: lastName(p.labOf) })}<br><small class="muted">${esc(peerAffiliation(s, p))}</small></span><span class="small">${t(p.status)}</span><span class="small">${bondWord(p.bond)}</span></div>`).join('')}</div>`)}
