import { chatphdApp as chatphd } from './chatphd.js';
import { esc, btn, bar, tag, group, note, band, money } from '../helpers.js';
import { icon } from '../icons.js';
import { venues, venueById, nextDeadline, acceptsThisMonth, selectivityLabel, tierLabel, fitsTopic, venuesForTopic, timelineFor } from '../../data/venues.js';
...
import { writeBudget, canSubmitNow, canSetTarget, canSkipApproval, responseDraft, reviewView } from '../../engine/paper.js';
import { composedText, isStreaming } from '../compose.js';
import { BENCH_ENERGY } from '../../data/bench.js';
import { CLUSTER_ENERGY } from '../../data/cluster.js';
...
function pipelineRail(p) {
  ...
  return `<div class="pipeline" role="list">${stages.map((name, i) => { const skipped = name === 'Advisor Review' && p.approvedWithout; return `<span class="pstage ${skipped ? 'skipped' : i < at ? 'done' : i === at ? 'now' : ''}" role="listitem"><i>${skipped ? '—' : i < at ? '✓' : i + 1}</i>${esc(skipped ? t('Advisor review skipped') : t(name))}</span>`; }).join('')}</div>`;
}
// Returns { title, detail, cta } — cta is pre-rendered HTML or ''.
function nextStep(s, p, here) {
  ...
  const writingStep = (title, detail) => {
    const n = Math.max(0, Math.min(writeBudget(s) - (s.typed || 0), 100 - p.draft));
    if (!n) return { title: t('Writing session complete'), detail: t('You have used this turn’s draft allowance. Choose your next plan in PhD Manager; Continue advances time.'), cta: btn(t('Choose next plan'), 'open', { app: 'dashboard', cls: 'primary' }) };
    return { title, detail, cta: go(t('Write for this turn (+{n})', { n }), 'write-session') };
  };
  if (p.kind === 'thesis') {
    if (p.status === 'Drafting' && p.draft >= 90) return { title: t('Send it to your advisor'), detail: t('The dissertation is ready for the committee to read.'), cta: go(t('Send to advisor'), 'send-advisor') };
    if (p.status === 'Drafting') return writingStep(t('Write the dissertation to 90%'), t('The committee reads it at 90%. You are at {n}%.', { n: Math.round(p.draft) }));
  ...
  if (p.status === 'Advisor Review') {
    const weeks = Math.max(0, p.reviewDueWeek - absWeek(s));
    // A reply at the next month's boundary is too late to submit to this target.
    const closingWeek = Number.isInteger(p.targetMonth) ? (p.targetMonth + 1) * 4 : null;
    if (p.targetVenueId && closingWeek !== null && p.reviewDueWeek >= closingWeek) return {
      title: t('The review may miss your deadline'),
      detail: t(canSkipApproval(s) ? 'Reply in {n} week(s); target closes in {d}. Skip the review at a cost to goodwill, or choose a later deadline.' : 'Reply in {n} week(s); target closes in {d}. Choose a later deadline, or wait until week 3 to skip the review at a cost to goodwill.', { n: weeks, d: Math.max(0, closingWeek - absWeek(s)) }),
      cta: go(t('Skip the advisor’s read'), 'skip-approval', { disabled: !plan || !canSkipApproval(s), title: t('You can only skip the advisor’s read late in a crunch, or when they are unreachable.') }) + (here === 'openregret' ? '' : btn(t('Review deadlines'), 'browser-tab', { id: 'openregret' })),
    };
    return { title: t('Your advisor is reading'), detail: t('Expected reply in {n} week(s). Choose a plan to keep going while you wait.', { n: weeks }), cta: btn(t('Choose next plan'), 'open', { app: 'dashboard', cls: 'primary' }) };
  }
  if (p.status === 'Ready') {
  ...
  if (p.draft < 60) return writingStep(t('Write the draft to 60%'), t('Your advisor reads it at 60% draft and 40% research. Draft {d}%, research {r}%.', { d: Math.round(p.draft), r: Math.round(p.progress) }));
...
function overgrief(s, ui) {
  ...
  const writingNote = !canType
    ? p.status === 'Advisor Review' ? t('This version is with your advisor. Choose another plan while you wait.')
      : p.status === 'Ready' ? t(isThesis ? 'The dissertation is ready for defense.' : 'This draft is ready. Submit it in OpenRegret.')
        : p.status === 'Rejected' ? t('Choose a revision in OpenRegret before writing again.')
          : ['Submitted', 'Rebuttal', 'Accepted', 'Abandoned'].includes(p.status) ? t('This manuscript is closed to editing. Follow the next step above.')
            : t('Research must reach 35% before drafting.')
    : !plan ? t('Writing is available once you are planning a turn.')
      : !remaining || p.draft >= 100 ? t('Writing session complete') : '';
  ...
  <div class="row between" style="margin-bottom:8px"><div><b style="font-size:var(--fs-md)">${esc(p.title)}</b> ${tag(t(p.status), p.status === 'Accepted' ? 'ok' : '')}<div class="small muted">${esc(topics[p.topic])} · ${p.collaborators.map(c => esc(lastName(c))).join(', ')} · ${isThesis ? 'thesis.tex' : 'main.tex'} · ${t('autosaved (allegedly)')}</div></div>${p.targetVenue ? `<span class="small">${t('Target')}: <b>${esc(p.targetVenue)}</b> · ${dateLabel(p.targetMonth)} ${btn(t('clear'), 'clear-target', { cls: 'small link', disabled: !plan })}</span>` : isThesis ? `<span class="small muted">${t('The committee is the venue.')}</span>` : `<span class="small muted">${t('No target deadline (set one in OpenRegret)')}</span>`}</div>
  ${pipelineRail(p)}
...
<span class="ln">14</span><b>\\end</b>{document}</pre><div id="typing-zone" class="typing-zone" tabindex="${plan && canType && remaining > 0 ? 0 : -1}" role="textbox" aria-label="${esc(t('Writing session'))}" aria-readonly="true">${!writingNote ? `${t('Click here and mash keys to write.')}<br><span class="muted">${t('{n} draft points left this {unit}. Typing moves progress, not quality.', { n: remaining, unit: s.tempo === 'week' ? t('week') : t('month') })}</span>` : esc(writingNote)}${!writingNote ? '<span class="cursor">▌</span>' : ''}</div></div>
  ...
  <div class="group" style="margin-top:8px">${bar(t('Draft'), p.draft)}${bar(t('Research'), p.progress, { cls: 'teal' })}<div class="row" style="margin-top:6px">${btn(t('Write a paragraph (+5)'), 'write', { disabled: !plan || !canType || !remaining || p.draft >= 100 })}${btn(t('Read for the afternoon (−{n})', { n: BENCH_ENERGY }), 'bench-start', { disabled: !plan || s.actions.bench || s.player.stats.energy < BENCH_ENERGY || !['Idea', 'Prototype', 'Experiments', 'Drafting'].includes(p.status), title: s.actions.bench ? t('You have already done that this turn.') : t('Five moments in the literature where the timing is the skill. Easier if you are good at this.') })}${btn(t('The job died (−{n})', { n: CLUSTER_ENERGY }), 'cluster-start', { disabled: !plan || s.actions.cluster || s.player.stats.energy < CLUSTER_ENERGY || !['Prototype', 'Experiments', 'Drafting'].includes(p.status), title: s.actions.cluster ? t('You have already done that this turn.') : t('Four logs, one reservation, and one line in each that is actually the problem.') })}${btn(t('“A general framework for…”'), 'hype', { disabled: !plan || !['Drafting', 'Experiments'].includes(p.status) || s.actions.hype, title: t('More hype, more novelty, more risk') })}${btn(isThesis ? t('Send to the committee') : t('Send to advisor'), 'send-advisor', { cls: 'primary', disabled: !plan || ['Advisor Review', 'Ready', 'Submitted', 'Rebuttal', 'Accepted', 'Abandoned'].includes(p.status) || (isThesis ? p.draft < 90 : (p.draft < 60 || p.progress < 40)), title: isThesis ? t('Needs a 90% draft') : t('Needs 40% research and 60% draft') })}${p.status === 'Advisor Review' ? btn(t('Skip the advisor’s read'), 'skip-approval', { disabled: !plan || !canSkipApproval(s), title: t('Late in a crunch, or when they are unreachable. Costs goodwill.') }) : ''}${isThesis ? '' : btn(t('Post preprint (arXive)'), 'preprint', { disabled: !plan || p.preprint || p.draft < 85, title: t('Draft ≥85%. A timestamp against scooping; a little hype.') })}</div>
  ...
  ${p.status === 'Advisor Review' ? note(t('{name} is reading. Expected reply in {n} week(s). Choose another plan while you wait.', { name: lastName(s.advisor.name), n: Math.max(0, p.reviewDueWeek - absWeek(s)) })) : ''}${p.status === 'Ready' ? note(isThesis ? t('Approved. Schedule the defense from PhD Manager.') : p.approvedWithout ? t('Ready to submit without the advisor’s final read. Open OpenRegret when a venue is open.') : t('Approved. Open the OpenRegret tab to submit when a venue is open.')) : ''}</div>
  ${history(p)}</div></div>`;
}
...
function venueTable(s, p) {
  ...
  return `<div class="listview venue-table"><div class="lv-head"><span>${t('Venue')}</span><span>${t('Category')}</span><span>${t('Next deadline')}</span><span>${t('Selectivity')}</span><span>${t('Fit')}</span><span></span></div>${list.map(({ v, at }) => { const open = canSubmitNow(s, v); const fit = v.topics.includes('any') ? t('any') : v.primary === p.topic ? t('primary') : t('partial'); return `<div class="lv-row"><span><b>${esc(v.name)}</b><span class="muted tiny"> (${t('{real}-like', { real: v.real })})</span></span><span class="small">${esc(v.category)} · ${tierWord(v)}</span><span class="small"><span class="venue-label">${t('Next deadline')}</span>${v.rolling ? t('Rolling') : dateLabel(at) + (at === s.month ? ` · ${t('open')}` : '')}</span><span class="small"><span class="venue-label">${t('Selectivity')}</span>${sel(v)}</span><span class="small"><span class="venue-label">${t('Fit')}</span>${fit}</span><span>${!v.rolling && at < 72 ? btn(p.targetVenueId === v.id ? t('Targeted') : t('Target'), 'set-target', { id: v.id, cls: 'small', disabled: !plan || p.targetVenueId === v.id || !canSetTarget(p), title: t('Aim the project at this deadline. The deadline month runs week by week.') }) : ''}${p.status === 'Ready' && open ? btn(t('Submit here'), 'wizard', { id: v.id, cls: 'small primary', disabled: !plan }) : ''}</span></div>`; }).join('')}</div>`;
}

function openregret(s, ui) {
  const receipt = ui.submissionReceipt;
  const submitted = receipt && receipt.seed === s.seed && s.projects.find(p => p.id === receipt.projectId
    && p.status === 'Submitted' && p.submissionHistory.length === receipt.attempt);
  if (submitted) {
    const timeline = submitted.timeline, venue = venueById[submitted.venueId];
    return `<div class="webpage publication-receipt" data-submission-receipt tabindex="-1" role="region" aria-labelledby="receipt-heading">
      <div class="receipt-brand">Open<span>Regret</span> <small>${t('Author console')}</small></div>
      <div class="receipt-stamp" aria-hidden="true">${icon('check', 28)}</div>
      <h1 id="receipt-heading">${t('Submission received')}</h1>
      <p class="receipt-title">${esc(submitted.title)}</p>
      <p class="receipt-venue">${esc(venue?.name)} · ${dateLabel(timeline.submitted)}</p>
      <p class="small muted">${t('It is in. You do not have to refresh this page.')}</p>
      <dl class="receipt-dates">
        ${timeline.rebuttal !== null ? `<div><dt>${t('Reviews and response window')}</dt><dd>${dateLabel(timeline.rebuttal)}</dd></div>` : ''}
        <div><dt>${t('Expected decision')}</dt><dd>${dateLabel(timeline.decision)}</dd></div>
      </dl>
      <p class="small">${t('The manuscript is locked for review. Your submission history keeps the receipt.')}</p>
      <div class="receipt-actions">${btn(t('Choose next plan'), 'receipt-plan', { cls: 'primary' })}${btn(t('View submitted manuscript'), 'receipt-view', { id: submitted.id })}</div>
    </div>`;
  }
  const p = activeProject(s), plan = s.stage === 'plan';
  ...
  if (p.status === 'Ready' && p.wizardStep > 0) body = `<div class="wizard-steps">${labels.map((x, i) => `<span class="${p.wizardStep === i ? 'current' : p.wizardStep > i ? 'done' : ''}">${i + 1}. ${x}</span>`).join('')}</div><div class="group">${p.wizardStep === 1 ? `<h2>${t('Confirm metadata')}</h2><p>${t('Title')}: ${esc(p.title)}<br>${t('Area')}: ${esc(topics[p.topic])}<br>${t('Venue')}: ${esc(v?.name)}<br>${t('Claims')}: ${p.hype > 45 ? t('rather ambitious') : t('within plausible limits')}.</p>` : p.wizardStep === 2 ? `<h2>${t('Confirm authors')}</h2><ol>${[s.player.name, ...p.collaborators].map(a => `<li>${esc(a)}</li>`).join('')}</ol><p class="muted small">${t('Everyone has contributed. Definitions vary.')}</p>` : p.wizardStep === 3 ? `<h2>${t('Upload manuscript')}</h2><div class="sunken center" style="padding:18px">${icon('paper', 28)}<br><b>manuscript_final_FINAL_v7${p.approvedWithout ? '_unapproved' : ''}.pdf</b><br><span class="muted small">${t('Generated from Overgrief · margins: probably fine')}</span></div>` : `<h2>${t('Are you absolutely sure?')}</h2><p>${t('Venue')}: <b>${esc(v?.name)}</b>. ${t('Decision expected {month}.', { month: timeline ? dateLabel(timeline.decision) : '—' })}${timeline?.rebuttal != null ? ` ${t('Reviews and rebuttal in {month}.', { month: dateLabel(timeline.rebuttal) })}` : ''}</p><p class="muted small">${t('Once submitted, the manuscript cannot be edited, un-read, or un-regretted.')}</p>`}<div class="row" style="margin-top:8px">${btn(p.wizardStep === 4 ? t('Submit manuscript') : t('Confirm & continue'), p.wizardStep === 4 ? 'submit' : 'wizard', { cls: 'primary', disabled: !plan })}</div></div>`;
    ...
    body = `<h2>${t('Reviews')} · ${esc(v?.name)} <span class="muted small">${t('mean {n}/10', { n: avg })}</span></h2>
      <div class="reviews">${p.reviewers.map((r, i) => { const view = reviewView(r, i); return `<div class="review"><span class="score">${r.score}<small>/10</small></span><b>${esc(view.name)}</b><div class="tiny muted">${t('confidence {n}/5', { n: r.confidence })}</div><p>${esc(view.text)}</p></div>`; }).join('')}</div>
      <div class="compose-pane rebuttal-pane">
        ...
        <div class="compose-body sunken" data-compose-scroll data-action="compose-skip" title="${esc(t('Click to skip the typing.'))}"><div class="compose-text ${chosen && isStreaming() ? 'streaming' : ''}" ${chosen ? 'data-compose-text' : ''}>${esc(chosen ? composedText() : '')}</div>${chosen ? '' : `<span class="compose-placeholder">${t('Choose an approach. The letter gets drafted for you.')}</span>`}</div>
  ...
  else body = note(p.status === 'Ready' ? p.approvedWithout ? t('Proceeding without the advisor’s final read. Pick an open venue below, or set a later target.') : t('Approved by your advisor. Pick a venue that is open this month, or set a target and wait for its deadline.') : p.status === 'Advisor Review' ? t('The draft is with your advisor. Set a target deadline below while you wait.') : t('Draft the paper in Overgrief and get advisor approval before submitting. You can set a target deadline any time.'));
...
function deadlinesPage(s, ui) {
  ...
  return `<div class="webpage"><h1>WhenIsThe<span style="color:#a11d1d">Deadline</span>.es</h1><p class="muted">${t('Upcoming deadlines for {topic}, derived from each venue’s real annual cycle. Countdown timers omitted for your health.', { topic: topics[s.player.profile.topic] })}</p>${note(t('Reference only: in-game dates are month-level projections, not live deadlines. Verify the official venue page before any real submission.'))}<table class="grid"><tr><th>${t('Venue')}</th><th>${t('Deadline')}</th><th>${t('In')}</th><th>${t('Decision')}</th><th>${t('Conference')}</th><th>${t('Selectivity')}</th></tr>${list.map(({ v, at }) => { const timeline = timelineFor(v, at, monthOf); return `<tr><td><b>${esc(v.name)}</b> <span class="muted tiny">${t('{real}-like', { real: v.real })}</span>${v.reference ? `<br><a class="tiny" href="${v.reference.source}" target="_blank" rel="noopener noreferrer">${esc(v.reference.cycle)} ↗</a>` : ''}</td><td>${dateLabel(at)}</td><td>${at === s.month ? `<b>${t('now')}</b>` : t('{n} mo', { n: at - s.month })}</td><td>${dateLabel(timeline.decision)}</td><td>${timeline.conference !== null ? dateLabel(timeline.conference) : '—'}</td><td>${sel(v)}</td></tr>`; }).join('')}</table></div>`;
}
...
export function rebuttalDraft(s, id) {
  return responseDraft(activeProject(s), id);
}
export const rebuttalComposeContext = p => ({ projectId: p.id, submitted: p.timeline?.submitted ?? null, attempt: p.submissionHistory?.length ?? 0 });
export function rebuttalComposeMatches(s, compose) {
  const p = activeProject(s);
  return !!(p?.status === 'Rebuttal' && compose?.kind === 'rebuttal' && compose.projectId === p.id
    && compose.submitted === (p.timeline?.submitted ?? null) && compose.attempt === (p.submissionHistory?.length ?? 0));
}
