import { backupControls, backupPreview } from './backups.js';
...
import { ordinaryExitEpilogues } from '../data/endings.js';
import { ventureEnding } from '../engine/venture.js';
import { personality, lastName, entryDate, entryText, peerAffiliation } from '../engine/state.js';
...
export function setupWizard(ui, meta, saved, notices) {
  ...
  const banner = `<div class="wizard-banner">${icon('wizard', 40)}<h2>Academic OS<br>${t('Setup')}</h2><small>US CS PhD Simulator<br>${t('No account · no server')}</small><small style="margin-top:auto">${meta.endings.length === 1 ? t('1 ending · {a} achievements found on this computer', { a: meta.achievements.length }) : t('{e} endings · {a} achievements found on this computer', { e: meta.endings.length, a: meta.achievements.length })}</small><small class="wiz-repo"><a href="${REPO_URL}" target="_blank" rel="noopener noreferrer">${esc(REPO_LABEL)}</a></small></div>`;
  ...
  if (step === 0) {
    ...
    <div class="radio-list" style="margin-top:8px">${opt('continue', saved?.phase === 'ending' ? t('Review the last run’s transcript') : t('Continue the saved run'), saved ? `${esc(saved.player.name)} · ${saved.phase === 'playing' ? dateLabel(saved.month) : saved.phase === 'ending' ? esc(saved.ending.title) : t(saved.phase)} · ${saved.program?.name || t('applying')}` : t('No saved run on this computer'), !saved)}${opt('new', t('New applicant'), t('Answer a short questionnaire about the person behind the application.'))}${opt('random', t('Randomize My Academic Fate'), t('Generate everything. Regret nothing, briefly.'))}${opt('collection', t('Achievements & discoveries'), t('What you have survived so far.'))}</div>${langRow}${notices.map(n => note(esc(t(n)))).join('')}`;
    buttons = `${btn(t('Next >'), 'wiz-next', { cls: 'primary', attrs: 'data-default="1"' })}`;
  } else if (step === 1) {
    page = `<h1>${t('Before you begin')}</h1><p>${t('This is a work of satire about academic life. The forms are fictional. The feeling may be familiar.')}</p>
    <div class="setup-notice"><p>${t('All people, schools, labs, employers, and venues are fictional or parodies. Names are generated; any resemblance is coincidental.')}</p><p>${t('The simulation is not academic, financial, medical, or immigration advice. It is not affiliated with any real program.')}</p></div>
    <details class="optional-block setup-disclaimer"><summary>${t('Full disclaimer')}</summary><p>${esc(t(DISCLAIMER))}</p></details>
    <label class="check setup-acknowledgement"><input type="checkbox" id="eula" ${ui.eula ? 'checked' : ''}> ${t('I understand that everyone in this game is fictional.')}</label>`;
...
export function endingApp(s, meta) {
  const venture = s.ending.id === 'spinout' ? ventureEnding(s) : null;
  const ending = venture ? { title: venture[0], text: venture[1] } : s.ending;
  ...
  const epilogues = { phd_faculty: t('Five years later, you are writing a grant at 2 a.m. and a student has just asked whether the change is small. You say yes.'), phd_industry: t('Five years later, you lead a team. Your calendar has a color for “thinking.” Nobody has ever booked it.'), phd_postdoc: t('Two years later, you are on the market again. You are better at it. The market is not.'), phd_startup: t('Three years later, the startup pivots to something adjacent to your thesis. The thesis is cited in the pitch deck, in a footnote.'), phd_open: t('That September, an offer arrives from a place you had not considered. You take it. It is fine. It is, honestly, more than fine.'), ...Object.fromEntries(Object.entries(ordinaryExitEpilogues).map(([id, text]) => [id, t(text)])) };
  // Current career runs already contain the player's five-year verdict. Do not append
  // a legacy two-year prediction after it (notably for the still-current postdoc ID).
  const epilogue = s.epilogue?.finished ? '' : epilogues[s.ending.id] || '';
  const relationship = s.advisor ? (s.relationship.conflict > 60 ? t('Strained') : s.relationship.trust > 65 ? t('Supportive') : t('Complicated')) : '—';
  return `<div class="transcript"><div class="hdr">${icon(s.ending.id.startsWith('phd_') ? 'grad' : s.ending.id === 'pass' ? 'star' : 'doc', 36)}<h1>${esc(ending.title)}</h1></div>
  <div class="ending-actions">${btn(t('Try another academic fate →'), 'new', { cls: 'primary' })}${btn(t('Achievements'), 'collection')}${btn(t('Saved runs'), 'saves')}</div>
  <p class="small muted">${t('The story is over. Explore its records in the desktop apps, or start a different story.')}</p>
  <details class="ending-story" data-disclosure="ending-story"><summary>${t('Read your ending')}</summary>${String(ending.text).split(/\n\s*\n/).map(p => `<p>${esc(p)}</p>`).join('')}${epilogue ? `<p class="small muted"><b>${t('Epilogue.')}</b> ${esc(epilogue)}</p>` : ''}</details>
...
export const aboutDialog = () => `<div class="modal"><section class="dialog narrow" role="dialog" aria-modal="true"><div class="titlebar"><span class="tb-title">${icon('info', 16)}<span>${t('About Academic OS')}</span></span></div><div class="body"><div class="row"><span>${icon('wizard', 40)}</span><div><b>Academic OS 1.12</b><br><span class="small muted">${t('US CS PhD Simulator · runs locally on your device')}</span></div></div><p class="small" style="margin-top:8px">${esc(t(DISCLAIMER))}</p><p class="tiny muted">${t('Venue timing uses representative official cycles from 2025–2027 and is projected by month into later fictional years. It is not a live deadline calendar; check each venue’s official site before a real submission.')}</p>
...
export const tipsDialog = (s = null) => {
  ...
  return `<div class="modal"><section class="dialog narrow" role="dialog" aria-modal="true"><div class="titlebar"><span class="tb-title">${icon('info', 16)}<span>${t('Welcome to Academic OS')}</span></span></div><div class="body"><h2>${applying ? t('How this starts') : t('Did you know…')}</h2><ul class="small">${tips.slice(0, 3).map(x => `<li>${x}</li>`).join('')}</ul><details class="optional-panel" data-disclosure="tips-more"><summary>${t('More guidance')}</summary><ul class="small">${tips.slice(3).map(x => `<li>${x}</li>`).join('')}</ul></details><label class="check"><input type="checkbox" id="tips-toggle" checked> ${t('Show tips at startup')}</label></div><div class="buttons">${btn(t('Close'), 'close-dialog', { cls: 'primary', attrs: 'data-default="1"' })}</div></section></div>`;
};
...
export function savesDialog(slots, live, ui) {
  ...
  return `<div class="modal"><section class="dialog saves" role="dialog" aria-modal="true" aria-labelledby="sv-title">
    ...
    <div class="body">
      ...
      ${ui.backupPreview ? backupPreview(ui.backupPreview, ui.saves.recoveryCount) : ` ${ui.saves.recoveryCount ? backupControls(ui.saves.recoveryCount) : ''}<div class="slot-list">${row(0, live, { live: true })}${slots.map(x => row(x.id, x.run)).join('')}</div>${ui.saves.recoveryCount ? '' : backupControls()}`}
      ${ui.backupPreview ? '' : `<p class="tiny muted">${t('Deleting a slot keeps your achievements. Restoring a backup replaces them with the achievements in that file.')}</p><div class="choices">${btn(t('Close'), 'saves-close', { cls: 'primary' })}${btn(t('Reset this game entirely…'), 'reset', { cls: 'small danger' })}</div>`}
    </div></section></div>`;
}

export const confirmDialog = ui => `<div class="modal"><section class="dialog narrow" role="dialog" aria-modal="true"><div class="titlebar"><span class="tb-title">${icon('warn', 16)}<span>Academic OS</span></span></div><div class="body"><h2>${ui.confirm === 'reset' ? t('Reset all saved progress?') : t('Start a new academic fate?')}</h2><p class="small">${ui.confirm === 'reset' ? t('This removes this game’s current run, saved slots, preserved originals, achievements, discoveries, and settings from this device.') : t('Your current run will be replaced. Achievements and discoveries remain.')}</p></div><div class="buttons">${btn(t('Cancel'), 'cancel-confirm')}${btn(ui.confirm === 'reset' ? t('Reset Save') : t('New Run'), 'confirm', { cls: 'primary', attrs: 'data-default="1"' })}</div></section></div>`;
