...
export function setupWizard(ui, meta, saved, notices) {
  ...
  const banner = `<div class="wizard-banner">${icon('wizard', 40)}<h2>Academic OS<br>${t('Setup')}</h2><small>US CS PhD Simulator<br>${t('Build 0.3 · offline · no account')}</small><small style="margin-top:auto">${t('{e} endings · {a} achievements found on this computer', { e: meta.endings.length, a: meta.achievements.length })}</small></div>`;
