...
export function setupWizard(ui, meta, saved, notices) {
  ...
  const banner = `<div class="wizard-banner">${icon('wizard', 40)}<h2>Academic OS<br>${t('Setup')}</h2><small>US CS PhD Simulator<br>${t('Build 1.3 · offline · no account')}</small><small style="margin-top:auto">${meta.endings.length === 1 ? t('1 ending · {a} achievements found on this computer', { a: meta.achievements.length }) : t('{e} endings · {a} achievements found on this computer', { e: meta.endings.length, a: meta.achievements.length })}</small><small class="wiz-repo"><a href="${REPO_URL}" target="_blank" rel="noopener noreferrer">${esc(REPO_LABEL)}</a></small></div>`;
