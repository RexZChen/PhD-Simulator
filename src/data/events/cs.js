...
export default [
  ...
  { id: 'cs_ten_days', title: 'The arrival checklist', category: 'life', scene: 'campus',
    forced: true, probability: 1, once: true, cooldown: 99, conditions: { international: true, maxMonth: 1 },
    text: 'Your arrival checklist sends you between the international office and payroll. Each asks for a document from the other. You need to know what is missing and when you will be paid.\n\nThe welcome slide says “We are here to help.” It does not say which building.',
    choices: [
      c('queue', 'Take the documents to the office in person', '$14 transport; spend energy to clarify the next step', { energy: -6, stress: -6, money: -14 },
        { result: 'At the desk, you work out which document goes where. You leave with a shorter list. This visit does not change the payroll date or your rent.' }),
      c('office', 'Ask the international office to untangle the checklist', 'Communication check; no travel cost', { energy: -3, stress: 6 },
        { check: { skill: 'communication', difficulty: 46 },
          successEffects: { stress: -8, hope: 4 }, failureEffects: { stress: 10, hope: -5 },
          successText: 'An adviser sends a checklist that explains the order. It has the same university logo as the contradictory ones, which is reassuring in a very limited sense.',
          failureText: 'The reply links to the page you started from. You now have evidence that the link works, but no clearer answer.' }),
      c('borrow', 'Buy the essentials and leave the forms for later', '$260 now; the paperwork remains unresolved', { stress: 8, money: -260, hope: -4 },
        { personality: 'grinder', result: 'You pay for immediate essentials. The paperwork is still waiting. At least there is now food beside the folder on the table.' }),
    ] },

  { id: 'cs_speak_screen', title: 'The teaching practice screening', category: 'department', scene: 'campus',
    forced: true, probability: 1, once: true, cooldown: 99, conditions: { international: true, minMonth: 1, maxMonth: 5 },
    text: 'At {school}, a teaching preparation session asks you to explain a sorting algorithm to two staff members and a camera. The task is to make the steps understandable, not to prove you know the algorithm.\n\nThey have a feedback form. You have a whiteboard. This session does not change your TA assignment or pay.',
    choices: [
      c('slow', 'Slow down and explain each step', 'Communication check; spend energy on clarity', { energy: -3 },
        { check: { skill: 'communication', difficulty: 45 },
          ...
          successText: 'A pause gives the listeners time to follow the example. The feedback identifies what worked. It is more useful than a box marked “satisfactory”.',
          failureText: 'The feedback asks for a clearer transition between steps. You know the material; making each link audible needs more practice.',
          flags: { accentCourse: 'onFail' } }),
      c('perform', 'Teach the room and ignore the camera', 'Teaching check; more energy and pressure', { energy: -5, stress: 6 },
        { check: { skill: 'teaching', difficulty: 52 },
          ...
          successText: 'You work through an example and check whether they are following. One staff member stops writing to answer. For a moment, it feels like teaching rather than recording evidence of teaching.',
          failureText: 'You skip the explanation of the partition step. A listener asks where that part came from. The camera has recorded the jump with excellent fidelity.' }),
      c('ask', 'Ask what the feedback criteria mean', 'Clarify the exercise; no performance check', { energy: -2, stress: -4, hope: 3 },
        { result: 'A staff member explains the criteria: sequence, examples, and checking understanding. You ask how those appear on the form. The answer takes longer than the headings suggest.' }),
    ] },
];
