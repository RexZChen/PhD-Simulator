...
export default [
  ...
  { id: 'group_praise_public', tone: 'supportive', title: 'Before we start', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 2,
    conditions: { flag: 'acceptPraise' },
    text: ['“Before we start — {you} had a paper accepted at {venue}.” A round of applause that lasts about four seconds. Then: “This is what I mean by seeing a project through.”\n\nThe applause is for you. The second sentence comes with eye contact for the rest of the table.',
      '“{you} got into {venue}.” They say the venue name slightly slowly, so that it lands. “A finished submission. That matters.” They look around the table on the last two words.\n\nTwo people write something down. One of them is not writing down congratulations.',
    ...
    choices: [
      ...
      c('uncomfortable', 'Notice what the compliment is being used for', 'Clear-eyed; slightly lonely', { hope: 2, confidence: 4, satisfaction: -2 },
        { flags: { acceptPraise: false }, personality: 'cynic',
          result: 'The compliment is real. So is the instruction to everybody else. You write down the venue name; the comparison can stay on the other side of the table.' }),
    ] },
  ...
  { id: 'group_round_thin', title: '“Let’s go round the table.”', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 4,
    ...
    choices: [
      ...
      c('deflect', 'Give a one-line update and hand off fast', 'Small; nobody remembers it', { energy: -2, satisfaction: -2 },
        { presented: true, result: 'Eleven words and a full stop. {advisor} pauses as if expecting a number, then calls on the next person. Your update is over; the empty space after it had excellent acoustics.' }),
    ] },
  { id: 'group_round_strong', title: 'It is your turn and you have something', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 4,
    actor: { type: 'labmate' }, conditions: { minMonth: 6, hasEditableProject: true, minProgress: 55 },
    text: 'Round the table, thirty seconds each. You have enough work behind {project} to explain the approach and what still needs checking. {labmate} closes their laptop to listen. The timer is less considerate.',
    choices: [
      c('plain', 'State it in one sentence and stop', 'The version that lands', { energy: -2, satisfaction: 6, confidence: 7 },
        { presented: true, labBond: 2, result: 'You state the approach and its main limitation, then stop. {advisor} asks one question about the next step. For once, the question fits inside the time slot.' }),
      c('long', 'Use three minutes to explain the details', 'More detail; less time for everyone else', { energy: -5, satisfaction: 3, hype: 4 },
        { presented: true, labBond: -3, result: 'You explain the details, then explain why each detail matters. The room follows you, but the next speaker is watching the clock. Their thirty seconds have started paying rent to yours.' }),
      c('credit', 'Invite {labmate} to question the next step', 'Share the floor; strengthen a real working relationship', { energy: -2, satisfaction: 4, confidence: 3 },
        { presented: true, labBond: 8, bond: 10, result: 'You ask {labmate} what they would check next. They ask you to spell out an assumption. It becomes a discussion instead of a performance, and you have not volunteered them to run the experiment.' }),
    ] },
  { id: 'group_public_correction', title: 'You are corrected in front of everyone', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 6,
    conditions: { minMonth: 10, minToxicity: 42, hasEditableProject: true, minProgress: 20 },
    text: ['Two minutes in, {advisor} interrupts: “That does not follow from what you have shown.” The room goes very quiet and very interested, in the way rooms do.',
      'You explain a step in {project}. {advisor} says “that cannot be right.” You have not yet established whether the objection holds. Everyone has established that it was loud.'],
    choices: [
      c('check', 'Check it live, on the screen, now', 'High risk; make the evidence visible', { energy: -5, stress: 12 },
        { check: { skill: 'research', difficulty: 55 },
          ...
          successText: 'You walk through the reasoning and identify the assumption behind the objection. On that point, your explanation holds. {advisor} says “fine — good.” The discussion moves to what still needs testing.',
          failureText: 'Walking through the argument exposes a gap. You cannot settle it in the room. The useful part is knowing what to check; the projector makes the discovery unusually public.',
          presented: true }),
      c('note', '“Let me check and come back to you.”', 'Defer the check; do not concede the claim', { energy: -2, stress: 4, satisfaction: -2 },
        { presented: true, result: 'You write down the objection and return to the agenda. The claim is still unverified. At least the next check now has a question attached to it, rather than a volume setting.' }),
      c('fold', 'Agree immediately and move on', 'Ends the exchange; leaves the evidence unchecked', { energy: -1, stress: -3, confidence: -6, novelty: -3 },
        { presented: true, personality: 'peoplePleaser', result: 'You say “oh — you are probably right.” The slide advances. Nobody has checked the reasoning, but the objection has become the last word on the claim.' }),
    ] },
  { id: 'group_laughed_at', title: 'Somebody laughs', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 8,
    ...
    choices: [
      c('carry', 'Carry on as though it did not happen', 'The professional move; it stays with you', { energy: -3, confidence: -5, stress: 6 },
        { presented: true, result: 'You finish your update. The senior student looks down at their phone. Nobody asks about the interruption; {advisor} asks about the baseline.' }),
      c('name', '“Sorry — was there something?”', 'Four words; the room changes shape', { energy: -3, stress: 8 },
        { check: { stat: 'confidence', difficulty: 60 },
          ...
          successText: 'They go red and say “no, sorry — carry on.” Later, by the kettle, they apologise without an audience. For the rest of this update, at least, you have the room.',
      ...
      c('after', 'Find them afterwards', 'Slower; usually better', { energy: -3, stress: -2 },
        { bond: 4, presented: true, result: 'They show you the message on their phone: the laugh was about that, not your update. “Terrible timing. Sorry.” A ninety-second conversation supplies the context the whole room was missing.' }),
    ] },
  { id: 'group_nobody_read', title: 'Nobody read it', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 3,
    conditions: { hasEditableProject: true, minDraft: 20 },
    text: ['You open the current draft of {project}. “Has anyone had a chance to read this?” A pause. The first question is “sorry, remind me what this is?”',
      'The draft is on the screen. Around the table, people start reading the opening at the same time. The meeting has acquired a live loading bar.'],
    choices: [
      ...
      c('rebook', 'Stop here and ask for a better-prepared discussion', 'Save energy now; no feedback or new booking yet', { stress: -2, energy: 2 }, { personality: 'boundarySetter', result: 'Everyone agrees that advance reading would help. No new time is booked. For now, the draft remains yours and the rest of this slot does not have to be.' }),
    ] },
  ...
  { id: 'group_someone_else', title: '{labmate} presents', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 2, actor: { type: 'labmate' },
    ...
    choices: [
      ...
      c('support', 'Say the thing that is working', 'A little energy; a specific word of support', { energy: -1 },
        { bond: 8, labBond: 2, result: 'You name the part that works and explain why. {labmate} stops defending the whole project long enough to discuss that result. The room has something useful to build on.' }),
  ...
  { id: 'group_derail', title: 'The meeting is about something else now', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 4,
    ...
    choices: [
      ...
      c('notes', 'Write down what they are actually arguing about', 'Find a useful question inside the argument', { energy: -2, novelty: 4, academicCapital: 2 },
        { personality: 'perfectionist', result: 'Underneath the argument is a disagreement about what counts as evidence. You write down the two standards side by side. The meeting has produced no decision, but your notes now contain a research question.' }),
    ] },
  ...
  { id: 'group_visitor', title: 'There is a visitor in the room', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 6, conditions: { minMonth: 8 },
    ...
    choices: [
      ...
      c('normal', 'Present the way you would anyway', 'Steady; occasionally that is the win', { energy: -5, readiness: 4, confidence: 2 },
        { presented: true, result: 'You do the normal version. Afterwards the visitor asks {advisor} who gave that update. Your name gets repeated, this time to somebody outside the lab.' }),
  ...
  { id: 'group_reading', title: 'Reading group, and nobody read', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 5,
    ...
    choices: [
      ...
      c('cancel', 'Suggest everybody read it and come back', 'The honest option, taken rarely', { energy: 3, stress: -3 },
        { labBond: 3, result: 'Several calendars open at once. Somebody asks which section to focus on before the next discussion. The room has agreed to do the reading; the PDF has yet to receive this news.' }),
    ] },
  ...
  { id: 'group_your_turn_again', title: 'It is your turn again', category: 'lab', scene: 'lab', probability: 1, scheduledOnly: true, cooldown: 3, conditions: { minMonth: 14 },
    text: ['The deck opens to a familiar title. There is one new figure. The figure is good and it is one figure.',
      'The calendar says “update.” The project has not consulted the calendar. You have one new figure and a slot long enough for an entire story.'],
    choices: [
      c('honest', 'Say out loud that there is not much new', 'Costs a little; buys the room back', { energy: -3 },
        { check: { stat: 'trust', difficulty: 45 }, successEffects: { trust: 5, satisfaction: 3, stress: -5 }, failureEffects: { satisfaction: -5, pressure: 5 },
          ...
          failureText: '“That is all?” {advisor} waits. Someone turns back to the figure, looking for a second result that is not there.', presented: true }),
